...
- [x] Chạy phân tích help/hurt trên 40 ảnh val với 4 feedback variants (binary / soft / none / oracle), mỗi variant 4 iterations — `analysis/feedback_analysis.py`, log tại `logs/feedback_analysis_log.txt`
- [x] Phân tích help/hurt theo từng cặp (image, iteration): binary so với none, kèm đặc trưng của mask feedback ở iteration trước (dice, error rate, mean uncertainty)
- [x] Phân tích error rate theo 10 bins của confidence trước threshold, tách riêng FP và FN
- [x] Diễn giải lại kết quả gradient flow: xác nhận fmask chưa từng được học bằng bằng chứng BN affine diff bằng đúng 0 (BN khởi tạo deterministic 1/0), tránh nhầm lẫn với nhiễu khởi tạo ở conv weights
- [x] Tổng hợp hai điểm yếu đã được xác nhận bằng dữ liệu và phác thảo hướng cơ chế feedback mới

## Findings quan trọng

### 1. Hard threshold cắt gradient hoàn toàn — fmask không bao giờ học

Nhánh fmask của MixPool dùng phép so sánh `(self.fmask(x) > 0.5).float()` nên đạo hàm bằng 0 tại mọi điểm. Phân tích gradient flow ghi nhận sau 10 training steps: 0/160 tham số fmask có gradient, trong khi conv1/conv2 đủ 160/160. Weight fmask không thay đổi sau optimizer step. So với model khởi tạo, BN affine (gamma/beta khởi tạo đúng 1/0) của fmask lệch đúng 0.000 — bằng chứng quyết định rằng nhánh này chưa từng được update trong toàn bộ quá trình train 53 epochs. Nói cách khác, "feature-based attention" mà paper quảng cáo thực chất là một bộ lọc random-init, toàn bộ sức mạnh của MixPool đến từ feedback mask.

### 2. Hard threshold vứt confidence — vùng không chắc chắn bị khuếch đại lỗi

Error rate của prediction đã binarize giảm monotonic theo confidence: 48.7% ở bin |p−0.5| < 0.1 (tương đương đoán mò) xuống 2.7% ở bin > 0.35. Threshold 0.5 đối xử pixel có xác suất sai 48.7% ngang hàng với pixel sai 2.7%, đưa cả hai vào feedback mask với trọng số bằng nhau. FP tập trung chủ yếu ở vùng confidence thấp (35.3% ở bin thấp nhất so với 0.0% ở bin cao nhất), FN phân bố đều hơn.
...
| Test-time refinement có cải thiện | 2 iters, 40 ảnh val | Jaccard 0.2166→0.2251, F1 0.3147→0.3268 | `results/test_results.csv` |
| Khi nào binary feedback giúp/hại; uncertainty correlate với lỗi? | 4 variants × 4 iters × 40 ảnh | 65 giúp / 46 hại; corr(prev_dice)=+0.355, corr(unc)=−0.297; err 48.7% (conf<0.1) vs 2.7% (conf>0.35) | `logs/feedback_analysis_log.txt` |

## Will Do (On going)

- [ ] Chạy lại evaluate.py đủ 10 iterations để có baseline chuẩn trước khi so sánh
- [ ] Confidence-gated feedback (trọng tâm): thay hard mask bằng feedback có trọng số theo confidence, giảm hoặc loại bỏ vùng |p−0.5| thấp
...
- Script feedback analysis: `analysis/feedback_analysis.py` — Script gradient flow: `analysis/grad_flow.py`
- Log feedback analysis: `logs/feedback_analysis_log.txt` — Log gradient flow: `logs/analysis_grad_log.txt`
- Data feedback analysis: `results/feedback_pairs.csv`, `results/feedback_uncertainty_bins.csv`, `results/feedback_summary.json`
- Baseline eval: `results/test_results.csv` — Checkpoint: `checkpoints/checkpoint.pth`
